...
async function main() {
  ...
  db.exec('CREATE INDEX idx_history_date ON djmdHistory(DateCreated)');
  db.exec('CREATE INDEX idx_content_date ON djmdContent(DateCreated)');
  db.exec('CREATE INDEX idx_sh_history ON djmdSongHistory(HistoryID)');
  db.exec('CREATE INDEX idx_sh_content ON djmdSongHistory(ContentID)');
  const indexEnd = performance.now();
  console.log(`Index creation time: ${(indexEnd - indexStart).toFixed(2)} ms`);
